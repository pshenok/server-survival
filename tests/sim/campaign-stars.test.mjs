...
describe("and the other levels speed could never carry", () => {
    ...
    //
    // L9 rejoined this set once its gateway was recalibrated (#276): a
    // waf -> apigw -> alb wiring with the tier throttles the burst instead of
    // hard-failing it, and the flawless-bonus path (rep >= 80 and >= 90) pays
    // the third star.
    const BUILDS = {
        3: () => {
        ...
        9: () => {
            // The gateway between the balancer and Compute, with the level's
            // own pre-built waf -> alb edge removed so ALL traffic is forced
            // through the limiter (otherwise the balancer round-robins around
            // it). A gateway sits BEFORE a balancer: waf -> apigw -> alb.
            const waf = svc("waf");
            const alb = svc("alb");
