...
function updateFinancesDisplay() {
  ...
  const serviceTypes = [
    ...
    },
    {
      ...
      label: "WebServer",
      color: "text-cyan-400",
      cost: CONFIG.services.webserver.cost,
    },
    {
      key: "alb",
      label: "ALB",
...
function spawnRequest() {
  ...
  if (conns.length > 0) {
    ...
    // If strict topology is enabled, enforce proper entry point
    if (CONFIG.networkTopology.strict) {
      // Priority order: WAF > ALB > WebServer
      const wafEntry = entryNodes.find((s) => s?.type === "waf");
      const albEntry = entryNodes.find((s) => s?.type === "alb");
      const webserverEntry = entryNodes.find((s) => s?.type === "webserver");

      if (wafEntry) {
        target = wafEntry;
      } else if (albEntry) {
        target = albEntry;
      } else if (webserverEntry) {
        target = webserverEntry;
      } else {
        // No valid entry point found
        target = null;
      }
    } else {
      // Original priority order for non-strict mode
      const wafEntry = entryNodes.find((s) => s?.type === "waf");
      const webserverEntry = entryNodes.find((s) => s?.type === "webserver");
      const albEntry = entryNodes.find((s) => s?.type === "alb");
    ...
    if (target) {
      // Record the initial hop in the request's path
      req.recordHop("internet");
      req.recordHop(target.type);
      req.flyTo(target);
    } else {
      failRequest(req);
    }
...
// Network topology validation function
function validateConnection(fromType, toType) {
  // If strict topology is disabled, allow all connections
  if (!CONFIG.networkTopology.strict) {
    return true;
  }

  // Define valid connections in the proper network flow
  const validConnections = {
    internet: ["waf", "alb", "webserver"],
    waf: ["alb", "webserver", "sqs"],
    alb: ["webserver", "sqs", "compute"],
    webserver: ["compute", "cache", "s3", "alb"], // Added ALB as valid destination
    sqs: ["alb", "compute"],
    compute: ["cache", "db", "s3"],
    cache: ["db", "s3"],
    db: [], // End of the line
...
// Find valid path through the network topology
function findValidPath(fromType, toType) {
  // If strict topology is disabled, return direct connection
  if (!CONFIG.networkTopology.strict) {
    return [fromType, toType];
  }

  // Define the proper network flow
  const networkFlow = [
    "internet",
    "waf",
    "alb",
  ...
  const fromIndex = networkFlow.indexOf(fromType);
  const toIndex = networkFlow.indexOf(toType);

  // If either type is not in the flow, return null
  if (fromIndex === -1 || toIndex === -1) {
    return null;
  }

  // If from is before to in the flow, return the path
  if (fromIndex < toIndex) {
    return networkFlow.slice(fromIndex, toIndex + 1);
  }

  // If bidirectional is enabled, allow reverse paths
  if (CONFIG.networkTopology.bidirectional) {
    return networkFlow.slice(toIndex, fromIndex + 1).reverse();
  }
...
function createConnection(fromId, toId) {
  ...
  const t1 = from.type || fromId;
  const t2 = to.type || toId;

  // Use the new validation function
  if (!validateConnection(t1, t2)) {
    new Audio("assets/sounds/click-9.mp3").play();
    console.error(
      `Invalid connection topology: ${t1} -> ${t2}. Expected flow: Internet -> WAF -> ALB -> WebServer -> Compute -> (DB/S3/Cache)`
    );
    return;
  }
...
container.addEventListener("mousedown", (e) => {
  ...
  if (STATE.activeTool === "select") {
    ...
    ["waf", "alb", "lambda", "db", "s3", "sqs", "cache", "webserver"].includes(
      STATE.activeTool
    )
  ) {
    // Handle upgrades for compute, db, cache, and webserver
    if (
      (STATE.activeTool === "lambda" && i.type === "service") ||
      (STATE.activeTool === "db" && i.type === "service") ||
      (STATE.activeTool === "cache" && i.type === "service") ||
      (STATE.activeTool === "webserver" && i.type === "service")
    ) {
      const svc = STATE.services.find((s) => s.id === i.id);
      if (
        svc &&
        ((STATE.activeTool === "lambda" && svc.type === "compute") ||
          ...
          (STATE.activeTool === "webserver" && svc.type === "webserver"))
      ) {
        svc.upgrade();
        return;
      }
  ...
  if (i.type === "service") {
    ...
    if (s) {
      ...
      if (
        ...
        (STATE.activeTool === "webserver" && s.type === "webserver")
      ) {
...
function analyzeFailure() {
  ...
  if (STATE.services.length < 3) {
    result.tips.push(
      "Build a complete pipeline: WAF → ALB → WebServer → Compute → DB/S3"
    );
  }

  if (!STATE.services.some((s) => s.type === "webserver")) {
    result.tips.push(
      "Add WebServer as the middle tier in your 3-tier architecture"
    );
  }
    ...
    setTimeout(() => {
      ...
      if (conns.length > 0) {
        ...
        if (CONFIG.networkTopology.strict) {
          ...
          const wafEntry = entryNodes.find((s) => s?.type === "waf");
          const albEntry = entryNodes.find((s) => s?.type === "alb");
          const webserverEntry = entryNodes.find(
            (s) => s?.type === "webserver"
          );
        ...
        if (target) {
          // Record initial hop in request's path
          req.recordHop("internet");
          req.recordHop(target.type);
          req.flyTo(target);
        } else {
